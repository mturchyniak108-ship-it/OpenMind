...
/*
 * Pre-generated locality workload.
 *
 * IMPORTANT:
 * Random-number generation happens BEFORE timing.
 * This makes the flat and local benchmarks comparable.
 */
vector<size_t> local_workload(
    size_t n,
    size_t data_size
) {
    vector<size_t> w(n);

    mt19937_64 rng(0xABCDEF1234567890ULL);

    size_t base = data_size / REGIONS;
    size_t rem  = data_size % REGIONS;

    for (size_t i = 0; i < n; ++i) {
        size_t region = i % REGIONS;

        size_t begin =
            region * base + min(region, rem);

        size_t length =
            base + (region < rem ? 1 : 0);

        uniform_int_distribution<size_t>
            dist(begin, begin + length - 1);
...
/*
 * Locality worker using pre-generated indices.
 */
Result local_worker(
