...
"""Non-scientific Phase 6D-Q3 fixture qualification.

This utility creates only the isolated Q3 active-generation authority and
fixture-authority evidence required by the frozen Q3 protocol.
...
def qualify_active_authority() -> tuple[dict[str, Any], str, bool]:
    if AUTHORITY.exists():
        require(
            ACTIVE_RECORD.is_file(),
            "fixture authority exists without active record",
        )

    if ACTIVE_RECORD.exists():
        existing = activation.reopen_active_generation(ACTIVE_RECORD)
        require(existing["model_pk"] == MODEL_PK, "existing active record model mismatch")
        require(existing["generation_pk"] == GENERATION_PK, "existing active record generation mismatch")
        require(
            existing["generation_manifest_sha256"] == MANIFEST_SHA256,
            "existing active record manifest mismatch",
        )

    segment_paths = {"segment:00000000": PHYSICAL_SEGMENT}
    result = activation.activate_generation(
        model_pk=MODEL_PK,
        generation_pk=GENERATION_PK,
        candidate_manifest_path=PHYSICAL_MANIFEST,
        active_record_path=ACTIVE_RECORD,
        segment_paths=segment_paths,
    )
    reopened = activation.reopen_active_generation(ACTIVE_RECORD)
    require(reopened == result.record, "active record reopen mismatch")
    ...
    require(reopened["generation_manifest_sha256"] == MANIFEST_SHA256, "active record manifest mismatch")
    return reopened, sha256_file(ACTIVE_RECORD), result.changed
...
def publish_authority(value: dict[str, Any]) -> tuple[str, bool]:
    ...
    if AUTHORITY.exists():
        existing = AUTHORITY.read_bytes()
        require(existing == raw, "existing fixture authority differs")
    ...
    try:
        os.link(AUTHORITY_PARTIAL, AUTHORITY)
        os.unlink(AUTHORITY_PARTIAL)
...
def main() -> int:
    ...
    active_record, active_sha, activation_changed = qualify_active_authority()
    ...
    print(f"ACTIVE_RECORD_CREATED={activation_changed}")
