"""Phase 6D telemetry snapshot persistence V1.1.
...
PERSISTENCE_V1_IMPLEMENTATION_SHA256 = (
...
class TelemetrySnapshotPersistenceBackendError(
    TelemetrySnapshotPersistenceError
):
    """Required immutable-publication backend is unavailable or unsupported."""
...
def _load_renameat2_backend() -> Any:
    try:
        libc = ctypes.CDLL(
            None,
            use_errno=True,
        ...
        raise TelemetrySnapshotPersistenceBackendError(
...
def _publish_snapshot_noreplace(
    *,
    partial: Path,
    final: Path,
    parent: Path,
    ...
    try:
        ...
        raise TelemetrySnapshotPersistenceWriteError(
            "parent directory open failed before publication; "
    ...
    try:
        renameat2 = _load_renameat2_backend()

        partial_name = os.fsencode(
            partial.name
        )

        final_name = os.fsencode(
            final.name
        )
        ...
        if rc != 0:
            ...
            if error_number == errno.EEXIST:
                raise TelemetrySnapshotPersistenceWriteError(
                    "final snapshot path appeared before atomic "
            ...
            if error_number:
                error_text = os.strerror(
                    error_number
                )

            else:
                error_text = "unknown errno"

            raise TelemetrySnapshotPersistenceWriteError(
                "atomic renameat2(RENAME_NOREPLACE) publication failed; "
                "partial residue preserved: errno "
                + str(
                    error_number
                )
                + ": "
        ...
        try:
            ...
            raise TelemetrySnapshotPersistenceWriteError(
                "final snapshot was published but parent directory "
                "fsync failed: "
                + str(exc)
            ) from exc
        ...
        if operation_error is None:
            operation_error = (
                TelemetrySnapshotPersistenceWriteError(
                    "parent directory descriptor close failed after "
