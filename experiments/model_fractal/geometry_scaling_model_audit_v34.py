...
OUTPUT = Path(
    "experiments/model_fractal/geometry_scaling_model_audit_v34.json"
)
...
def fit_power_original_space(xs, ys):
    """
    Directly minimize original-space SSE for:

        y = A * x^alpha

    For fixed alpha, the least-squares A is:

        A = sum(y*x^alpha) / sum(x^(2*alpha))

    Therefore only alpha requires numerical optimization.
    """
    ...
    # Broad physically meaningful search interval.
    lo = -5.0
    hi = 5.0
    ...
    fc, _ = evaluate_alpha(c)
    fd, _ = evaluate_alpha(d)
...
def fit_model(model, xs, ys):
    ...
    if model == "linear":
        params = fit_linear(xs, ys)
        parameter_count = 2

    elif model == "quadratic":
        params = fit_quadratic(xs, ys)
        parameter_count = 3

    elif model == "power":
        params = fit_power_original_space(xs, ys)
        parameter_count = 2
...
result = {
    "schema":
        "openmind.geometry_scaling_model_audit.v34",
...
print("=" * 72)
print(" OPENMIND / GEOMETRY SCALING MODEL AUDIT V34")
print("=" * 72)
print()
print("Layers:", layers)
...
print()
print("POWER FIT: DIRECT ORIGINAL-SPACE SSE")
print()
...
for window in WINDOWS:
    ...
    params = real_fits[window]["power"]["parameters"]
    log_a, alpha = params
