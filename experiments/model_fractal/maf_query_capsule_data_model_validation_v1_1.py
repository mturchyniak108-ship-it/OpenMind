...
PREDECESSOR_V1_PROTOCOL_SHA256 = (
...
ARM_ENV = (
    "OPENMIND_ARM_QUERY_CAPSULE_DATA_MODEL_VALIDATION_V1_1"
)
...
VALIDATION_PROTOCOL_PATH = (
    BASE
    / "MAF_QUERY_CAPSULE_DATA_MODEL_VALIDATION_V1_1_PROTOCOL.md"
)
...
PREDECESSOR_V1_PROTOCOL_PATH = (
    BASE
    / "MAF_QUERY_CAPSULE_DATA_MODEL_VALIDATION_V1_PROTOCOL.md"
)

PREDECESSOR_V1_RUNNER_PATH = (
    BASE
    / "maf_query_capsule_data_model_validation_v1.py"
)

PREDECESSOR_V1_PARTIAL_PATH = (
    BASE
    / "maf_query_capsule_data_model_validation_v1.json.partial"
)

PREDECESSOR_V1_VERDICT_PATH = (
    BASE
    / "MAF_QUERY_CAPSULE_DATA_MODEL_VALIDATION_V1_VERDICT.md"
)

PREDECESSOR_V1_FINAL_PATH = (
    BASE
...
RESULT_PATH = (
    BASE
    / "maf_query_capsule_data_model_validation_v1_1.json"
)
...
def verify_authorities() -> None:
    expected = (
        ...
        (
            PREDECESSOR_V1_PROTOCOL_PATH,
            PREDECESSOR_V1_PROTOCOL_SHA256,
            "predecessor V1 protocol",
        ),
        (
            PREDECESSOR_V1_RUNNER_PATH,
            PREDECESSOR_V1_RUNNER_SHA256,
            "predecessor V1 runner",
        ),
        (
            PREDECESSOR_V1_PARTIAL_PATH,
            PREDECESSOR_V1_PARTIAL_SHA256,
            "predecessor V1 partial evidence",
        ),
        (
            PREDECESSOR_V1_VERDICT_PATH,
            PREDECESSOR_V1_VERDICT_SHA256,
            "predecessor V1 verdict",
        ),
        (
            CHECKPOINT_PATH,
            CHECKPOINT_SHA256,
    ...
    if PREDECESSOR_V1_FINAL_PATH.exists():
        raise RuntimeError(
            "predecessor V1 final result must remain absent"
        )
...
def authority_snapshot() -> dict[str, str]:
    return {
        ...
        str(PREDECESSOR_V1_PROTOCOL_PATH):
            sha_file(PREDECESSOR_V1_PROTOCOL_PATH),

        str(PREDECESSOR_V1_RUNNER_PATH):
            sha_file(PREDECESSOR_V1_RUNNER_PATH),

        str(PREDECESSOR_V1_PARTIAL_PATH):
            sha_file(PREDECESSOR_V1_PARTIAL_PATH),

        str(PREDECESSOR_V1_VERDICT_PATH):
            sha_file(PREDECESSOR_V1_VERDICT_PATH),
...
def _load_renameat2_backend():
    try:
        ...
        raise RuntimeError(
            "failed to load process C library "
            "for renameat2 backend"
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "required libc renameat2 symbol "
            "is unavailable"
        ) from exc
...
def _rename_noreplace(
    ...
    if partial.parent.resolve() != final.parent.resolve():
        raise RuntimeError(
            "partial and final must share parent"
        )
...
def qualify_publication_backend() -> dict[str, bool]:
    _load_renameat2_backend()

    with tempfile.TemporaryDirectory(
        prefix=".openmind-v1_1-pubqual-",
        dir=str(BASE),
    ) as tmp_raw:
        parent = Path(tmp_raw)
        ...
        if partial.exists():
            raise RuntimeError(
                ...
                "did not consume partial"
            )

        if not final.exists():
            raise RuntimeError(
                "successful publication "
                "did not create final"
            )
        ...
        if not stat.S_ISREG(
            ...
            raise RuntimeError(
                "published final is not regular"
            )
        ...
        if final_stat.st_ino != source_inode:
            raise RuntimeError(
                "rename publication changed inode"
            )
        ...
        if not collision_rejected:
            raise RuntimeError(
                "RENAME_NOREPLACE collision "
                "was not rejected"
            )

        if (
            collision_final.read_bytes()
            != incumbent
        ):
            raise RuntimeError(
                "collision changed final bytes"
            )

        if (
            collision_partial.read_bytes()
            != candidate
        ):
            raise RuntimeError(
                "collision changed partial bytes"
            )

        if (
            collision_final.stat().st_ino
            != incumbent_inode
        ):
            raise RuntimeError(
                "collision replaced final inode"
            )

        return {
            "backend_available": True,
            "success_publication": True,
            "partial_consumed": True,
            "bytes_preserved": True,
            "inode_preserved": True,
            "directory_fsync": True,
            "collision_rejected": True,
            "collision_final_preserved": True,
            "collision_partial_preserved": True,
        }
...
def publish_result(
    ...
    if PARTIAL_PATH.exists():
        raise RuntimeError(
            "successful publication left partial"
        )

    if not RESULT_PATH.is_file():
        raise RuntimeError(
            "successful publication missing final"
        )
...
def main() -> int:
    ...
    if args.qualify_publication_backend:
        verify_authorities()

        if RESULT_PATH.exists():
            raise RuntimeError(
                "result exists during backend qualification"
            )

        if PARTIAL_PATH.exists():
            raise RuntimeError(
                "partial exists during backend qualification"
            )

        details = (
            qualify_publication_backend()
        )

        print(
            "STATUS=VALIDATION_V1_1_"
            "PUBLICATION_BACKEND_QUALIFICATION_PASS"
        )
    ...
    print(
        "OPENMIND / QUERY CAPSULE DATA MODEL VALIDATION V1.1"
    )
