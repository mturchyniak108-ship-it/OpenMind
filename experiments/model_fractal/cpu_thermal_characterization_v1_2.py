...
PROTOCOL = Path(
    ...
    "CPU_THERMAL_CHARACTERIZATION_V1_2_PROTOCOL.md"
)

RESULT = Path(
    "experiments/model_fractal/"
    "cpu_thermal_characterization_v1_2.json"
)

EXPECTED_PROTOCOL_SHA256 = "b98e4c4d7309948b0fbf71f4a961e038a7e55470f03153e85ec724ffd32f3cbb"
...
class CPUCharacterizationExecutionError(RuntimeError):
    def __init__(
        self,
        execution_error,
        affinity_restore_error,
        partial_evidence,
    ):
        ...
        super().__init__(
            " | ".join(parts)
            or "CPU characterization execution failed"
        )
...
def discover_thermal_zones():
    ...
    excluded = [
        ...
        if any(
            ...
            for word in THERMAL_EXCLUDE_KEYWORDS
        )
    ]
    ...
    if preferred:
        return (
            preferred,
            "keyword_preferred_trip_excluded",
            excluded,
        )
    ...
    if live_candidates:
        return (
            live_candidates,
            "all_live_plausible_fallback_trip_excluded",
            excluded,
        )

    raise RuntimeError(
        "no plausible non-trip thermal sensors"
    )
...
def run_characterization():
    ...
    zones, zone_mode, excluded_zones = (
    ...
    try:
        ...
        try:
            ...
            if not affinity_restored:
                raise RuntimeError(
                    "original CPU affinity was not restored"
                )

        except Exception as exc:
            affinity_restore_error = exc
            affinity_restored = False

    if (
        execution_error is not None
        or affinity_restore_error is not None
    ):
        failure_evidence = {
            "original_affinity":
                original_affinity,
        ...
        raise CPUCharacterizationExecutionError(
            execution_error,
            affinity_restore_error,
            failure_evidence,
        )
